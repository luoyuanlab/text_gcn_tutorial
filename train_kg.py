...
# Load data
adj, word_entity_adj, entity_adj_list, y_train, y_val, y_test, train_mask, val_mask, test_mask, train_size, test_size = load_corpus_kg(
    FLAGS.dataset)

print(adj.shape)
print(val_mask, len(val_mask))
doc_word_feat = sp.identity(adj.shape[0])
doc_word_nonzero_feat, doc_word_num_feat = doc_word_feat.shape
doc_word_feat = sparse_to_tuple(doc_word_feat.tocoo())

adj = preprocess_adj(adj)

entity_word_adj = word_entity_adj.T
word_entity_adj = preprocess_graph(word_entity_adj)
entity_word_adj = preprocess_graph(entity_word_adj)

n_entities = entity_adj_list[0].shape[0]
entity_feat = sp.identity(n_entities)
#entity_feat = wordnet_defs()
entity_nonzero_feat, entity_num_feat = entity_feat.shape
print(entity_nonzero_feat, entity_num_feat)
entity_feat = sparse_to_tuple(entity_feat.tocoo())

entity_adj_list = [preprocess_adj(x) for x in entity_adj_list]
...
nonzero_feat = {
    1: entity_nonzero_feat,
    0: doc_word_nonzero_feat,
}
feat = {
    1: entity_feat,
    0: doc_word_feat,
