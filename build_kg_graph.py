        ...
        if synset_str in synset_id_dict:
            ...
            for doc in doc_list:
                if doc < train_size:
                    ...
                    row.append(doc + vocab_size)
                    col.append(int(syn_id))
                    weight.append(1.0)                    

            word_entity_link_count += 1

word_entity_adj = sp.csr_matrix(
    (weight, (row, col)), shape=(train_size + vocab_size + test_size, n_entities))
...
for i in range(n_rel_types):
    # a mat for each relation
    mat = np.identity(n_entities)
    entity_adj_list.append(mat)

for triple in all_triples:
    temp = triple.split()
    entity_adj_list[0][int(temp[0]), int(temp[1])] = 1

for i in range(n_rel_types):
    mat = entity_adj_list[i]
    mat = sp.csr_matrix(mat)
    entity_adj_list[i] = mat
...
f = open("data/ind.{}.word_entity_adj".format(dataset), 'wb')
pkl.dump(word_entity_adj, f)
f.close()

f = open("data/ind.{}.entity_adj_list".format(dataset), 'wb')
pkl.dump(entity_adj_list, f)
f.close()
